...
import { InsertUser, notes, users } from "../drizzle/schema";
...
export async function listNotesByUserId(userId: number) {
  const db = await getDb();
  if (!db) throw new Error("資料庫暫時無法使用。");
  return db.select().from(notes).where(eq(notes.userId, userId)).orderBy(desc(notes.createdAt));
}

export async function createNote(input: { userId: number; content: string; source: string }) {
  const db = await getDb();
  if (!db) throw new Error("資料庫暫時無法使用。");
